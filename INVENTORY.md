...
### checkers（中国跳棋，星形棋盘 2–6 人）
- 客户端：选子、点目标、动画。
- 服务端：合法落点（邻接+跳跃，坐标几何在 grid 棋盘上）、轮次、到达目标区判胜。
- 注意：现有棋盘是几何星形（row/index 坐标 + 距离计算），规则须原样搬到服务端模块，不改玩法。

### ludo（飞行棋）
- 客户端：点掷骰按钮、选棋子。
- 服务端：骰子结果、起飞（=6）、移动、到达、全部到达判胜、轮次（现有规则无额外回合/吃子——保持简化版）。
...
`npm test` 12/12 通过（HEAD e14b532）。每游戏迁移须新增：
1. 规则单元测试（纯函数）；
