 ...
 *
 * 权威边界：
 * - 服务器持有队伍/飞机位置、轮次、骰子与胜负；客户端只发操作意图
 *   （roll / move / restart）；
 * - 骰子必须由服务器产生（crypto.randomInt）：客户端在 action 里夹带的任何
 *   dice 字段一概不采纳；
 * - 规则推进委托给纯规则模块 ludo-rules.js（服务端与浏览器共用同一份）；
 * - 房主创建房间时选定玩家数（playerCount），满员即自动开局；
 * - 快照字段与改造前 ludo.js 的客户端状态对齐，渲染层可直接复用。
 */
"use strict";
...
const REJECTION_TO_ERROR = Object.freeze({
  ...
  [rules.MoveRejection.DICE_NOT_ROLLED]: ErrorCodes.INVALID_ACTION,
  [rules.MoveRejection.BAD_PIECE]: ErrorCodes.INVALID_MOVE,
});
...
const REJECTION_DETAIL = Object.freeze({
  ...
  [rules.MoveRejection.DICE_NOT_ROLLED]: "roll the dice first",
  [rules.MoveRejection.BAD_PIECE]: "piece index out of range",
});
 ...
 */
class LudoRoom extends RoomBase {
  /**
  ...
  seatPlayer(player, role) {
    ...
    if (this.isFull() && !this.game.started) {
      this._setupGame();
      this.game.message = `${this.game.teams.length} 人飞行棋开始，掷到 6 可以起飞`;
    } else {
  ...
  gameSnapshot() {
    const g = this.game;
    return {
      teams: g.teams.map((team) => ({ ...team, pieces: [...team.pieces] })),
      turn: g.turn,
      dice: g.dice,
      started: g.started,
      over: g.over,
   ...
   *
   * 支持的动作：
   * - `roll`：当前回合方掷骰（骰子由服务器 crypto 生成，重复掷/他人掷被拒）；
   * - `move`：{pieceIndex} 移动本方飞机（服务器按自己保存的骰子点数推进）；
   * - `restart`：房主重新开局（清盘、轮次归零）。
   *
   * 注意：action 里夹带的任何其它字段（teams/turn/dice/winner/...）一概不采纳。
   *
  ...
  _applyRoll(seatIndex, ctx) {
    const g = this.game;
    // 1. 状态校验。
    if (!g.started) return { ok: false, code: ErrorCodes.GAME_NOT_STARTED, detail: REJECTION_DETAIL[rules.MoveRejection.NOT_STARTED] };
    if (g.over) return { ok: false, code: ErrorCodes.GAME_ALREADY_FINISHED, detail: REJECTION_DETAIL[rules.MoveRejection.FINISHED] };
    // 2. 轮次校验。
    if (seatIndex !== g.turn) return { ok: false, code: ErrorCodes.NOT_YOUR_TURN, detail: REJECTION_DETAIL[rules.MoveRejection.NOT_YOUR_TURN] };
    // 3. 重复掷骰校验（骰子未消耗前不能再掷）。
    if (g.dice !== 0) return { ok: false, code: ErrorCodes.INVALID_ACTION, detail: "dice already rolled" };
    // 4. 服务器权威骰子：crypto 随机 1..6（客户端夹带的 dice 字段不采纳）。
    g.dice = crypto.randomInt(1, 7);
    g.message = `${g.teams[g.turn].name} 掷出 ${g.dice}。`;
    this.touch();
   ...
   *
   * @param {number} seatIndex - 操作者座位下标。
   * @param {object} action - {action:'move', pieceIndex:number}。
   * @param {{playerId: string, requestId: string|null}} ctx - 发起者上下文。
   * @returns {{ok: boolean, code?: string, detail?: string}} 结果。
   */
  _applyMove(seatIndex, action, ctx) {
    const g = this.game;
    // 1. 交给纯规则模块判定（状态、轮次、骰子、下标一次性校验）。
    //    骰子只取服务器自己保存的 g.dice，action.dice 一概不采纳。
    ...
    // 2. 推进权威状态（记录本次骰子与起点，便于客户端提示文案）。
    //    team 引用指向推进前的旧数组对象，其 pieces 即移动前位置。
    const team = g.teams[g.turn];
    const pieceIndex = Number(action.pieceIndex);
    const from = team.pieces[pieceIndex];
    const applied = rules.applyMove(g.teams, g.turn, pieceIndex, g.dice);
    g.teams = applied.teams;
    g.moves.push({ team: team.color, pieceIndex, dice: g.dice, from, launched: applied.launched });
    // 3. 消耗骰子并结算。
    g.dice = 0;
    if (applied.won) {
      g.over = true;
      g.winner = team.color;
      g.message = `${team.name} 全部到达终点，获胜！`;
      this.status = "finished";
    } else {
      g.turn = applied.nextTurn;
      g.message = applied.launched
        ? `${team.name}${pieceIndex + 1} 号飞机起飞。`
        : applied.consumed
          ? `${team.name}${pieceIndex + 1} 号移动完成。`
          : `${team.name} 掷非 6 无法起飞，轮空。`;
    }
    this.touch();
    this.broadcastGame(ctx);
  ...
  _applyRestart(player, ctx) {
    ...
    // 3. 清盘重开。
    this._setupGame();
    this.game.message = "已重新开局，掷到 6 可以起飞";
    this.touch();
    this.broadcastGame(ctx);
  ...
  _setupGame() {
    ...
    g.teams = rules.createTeams(this.playerCount).map((team, index) => ({
      ...team,
      name: this.seats[index]?.nickname || team.name,
    }));
    g.turn = 0;
    g.dice = 0;
    g.started = true;
